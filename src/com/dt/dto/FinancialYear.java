...
public class FinancialYear implements Comparable<FinancialYear>{
    ...
    private final static Logger logger = 
            Logger.getLogger(FinancialYear.class.getName());
    
    public FinancialYear(LocalDate startDate, LocalDate endDate) {
        if (endDate.isEqual(startDate) || endDate.isBefore(startDate)) {
            throw new IllegalArgumentException("End Date must be a later date than Start Date!");
        }
    ...
    public boolean isYearOverLapped(FinancialYear year) {
        ...
        if ((otherStartDate.compareTo(startDate) >= 0 && 
                otherStartDate.compareTo(endDate) <= 0) || 
             (otherEndDate.compareTo(startDate) >= 0 &&
                otherEndDate.compareTo(endDate) <= 0)) {
            return true;
        }
    ...
    public static FinancialYear getOverlappingYear(List<FinancialYear> years, 
            FinancialYear year) {
     ...
     * @return If this financial year comes chronologically before the specified financial year
     */
        ...
        for(int i = 0; i < 2; i++) {
            ...
            try {
                ...
                logger.logp(Level.SEVERE, FinancialYear.class.getName(), "parse",
                        "Error in parsing a string for a financial year name", e);
                return null;
            }
            
            //convert milliseconds to a LocalDate object
            dates[i] = Instant.ofEpochMilli(millis)
    ...
    public String toEpochMillis() {
        ...
        long startMillis = startDate.atTime(LocalTime.MIDNIGHT)
                .atZone(ZoneId.of("Asia/Kolkata")).toInstant().toEpochMilli();
        
        long endMillis = endDate.atTime(LocalTime.MIDNIGHT)
                .atZone(ZoneId.of("Asia/Kolkata")).toInstant().toEpochMilli();
    ...
    public static void sort(final List<FinancialYear> years, boolean sortAscending) {
        ...
        if (sortAscending) {
            ...
            Collections.<FinancialYear>sort(years, 
                    Collections.<FinancialYear>reverseOrder());
        }
    }
