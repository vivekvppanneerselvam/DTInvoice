...
public class PaymentPersistence {
     ...
     private final static Logger logger = 
            Logger.getLogger(PaymentPersistence.class.getName());
    ...
    public static void savePayment(Payment payment) throws Exception {
        ...
        try {
            ...
            logger.logp(Level.SEVERE, PaymentPersistence.class.getName(),
                    "savePayment", "Error in saving the payment", e);
            if (connection != null) {
                 connection.rollback();
            }
            throw e;
    ...
    private static int saveNewPayment(Payment payment, Connection connection)
            throws SQLException {
    ...
    private static void saveInstrumentDetails(Payment payment, Connection connection)
            throws SQLException {
         String sql = "INSERT INTO payment_details VALUES (?, ?, ?, ?, ?)";
        try(PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setInt(1, payment.getPaymentId());
            ps.setString(2, payment.getInstrumentNumber());
            ps.setDate(3, Date.valueOf(payment.getInstrumentDate()));
   ...
   private static void updatePayment(Payment payment, Connection connection) 
       throws SQLException {
   ...
   private static void deleteInstrumentDetails(int paymentId, Connection connection)
           throws SQLException {
    ...
    public static Payment getPayment(int paymentId) throws SQLException {
        ...
        sql.append("SELECT p.*, c.name, pd.* ")
                .append("FROM payments p INNER JOIN customers c ")
                .append("ON p.customerid = c.cust_id ")
                .append("LEFT JOIN payment_details pd ")
                .append("ON p.id = pd.paymentid ")
                .append("WHERE p.id = ").append(paymentId);
        ...
        try {
                ...
                try(ResultSet rs = s.executeQuery(sql.toString())) {
                    if (rs.next()) {
                        ...
                        if (payment.getPaymentMode() == PaymentMode.CHEQUE || 
                                payment.getPaymentMode() == PaymentMode.DD) {
           ...
           logger.logp(Level.SEVERE, PaymentPersistence.class.getName(),
                    "getPayment", "Error in getting the payment details", e);
            throw e;
        }
