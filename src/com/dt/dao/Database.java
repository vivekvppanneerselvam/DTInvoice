...
public abstract class Database {
    private static final String DB_NAME = "dtinvoice";
    ...
    public static Connection getConnection(FinancialYear year, 
            boolean createDatabase) {
        String path = Global.getExtendedAppDataPath() + File.separator +
                year.toEpochMillis();
    ...
    public static Connection getConnection(String pathToDatabase, 
        ...
        final String connectionString = "jdbc:derby:" + pathToDatabase + 
        ...
        Properties properties = getProperties(createDatabase);
         ...
         try {
            ...
            logger.logp(Level.SEVERE, Database.class.getName(),
                    "getConnection", "Error in connecting to database", e);
            return null;
        }
    ...
    private static Properties getProperties(boolean createDatabase) {
        ...
        properties.setProperty("user", "vivek");
        properties.setProperty("password", "admin");
        
        if (!createDatabase) {
            /* Property to upgrade the database to the connecting version 
                of Derby, if it is newer */
            properties.setProperty("upgrade", "true");
        } else {
            //property to create a database
            properties.setProperty("create", "true");
            //set the locale to Indian
            properties.setProperty("territory", "en_IN"); 
            
            /* Create a case- and accent-insensitive database
             PRIMARY means case-insensitive and accent-insensitive
            SECONDARY means case-insensitive but accent-sensitive
            TERTIARY means case-sensitive and accent-sensitive */
    ...
    private static boolean setSchema(Connection connection) {   
        ...
        try(Statement s = connection.createStatement()) {
            s.execute(sql);
             ...
             logger.logp(Level.SEVERE, Database.class.getName(),
                    "setSchema", "Error in setting the default schema", ex);
             return false;
        }
            ...
            if (e instanceof SQLException) {
               ...
               */
                if (! (sqlState.equalsIgnoreCase("XJ015") || 
                    ...
                    logger.logp(Level.SEVERE, Database.class.getName(),
                    "shutDown", "Error in shutting down the Derby database", e);
                    return false;
                }
            }
        }
    ...
    public static boolean shutDownActiveYearDatabase() {
        ...
        if (Global.getActiveFinancialYear() != null) {
             final String pathToDatabase = Global.getExtendedAppDataPath() + 
                File.separator + Global.getActiveFinancialYear().toEpochMillis();
    ...
    public static boolean createDatabaseSchema(final Connection connection) {
       ...
       final String resourcePath = "/resources/sql/db-ddl.sql"; 
      //get the default encoding of the JVM
       final String fileEncoding = "UTF-8";
       
       try(InputStream sql = new BufferedInputStream(
              Database.class.getResourceAsStream(resourcePath))){
           ...
           try {
                int errorsEncountered = 
                    ij.runScript(connection, sql, fileEncoding, System.out, null);
                
                if (errorsEncountered > 0) {
                    final String str = errorsEncountered + " errors encountered while " +
                            "running the sql script";
                    throw new RuntimeException(str);
                }
           } catch (Exception e) {
               logger.logp(Level.SEVERE, Database.class.getName(), 
                      "createDatabaseSchema" , "Error in running the sql script", e);
               return false;
           }
           
        } catch (Exception ex) {
            logger.logp(Level.SEVERE, Database.class.getName(), 
                      "createDatabaseSchema" , "Error in accessing the SQL file resource", ex);
               return false;
        }
    ...
    public static boolean transferData(final FinancialYear newYear, 
            final FinancialYear existingYear) {
        ...
        try(Connection newDB = getConnection(newYear, false)) {
            ...
            try(Connection existingDB = getConnection(existingYear, false) ) {
                if (existingDB == null) {return false;}
    ...
    private static void transferFirmDetails(final Connection newDB, 
            final Connection existingDB) throws Exception {
        
       final String tempFile =  Global.getTempDirectoryPath() + File.separator + 
               "firm_details.dat";
        Files.deleteIfExists(Paths.get(tempFile));
       
        //command to export the table contents to a disk file
        StringBuilder sb = new StringBuilder("{CALL SYSCS_UTIL.SYSCS_EXPORT_TABLE(")
                .append("'APP', 'FIRM_DETAILS', '")
       ...
       sb = new StringBuilder("{CALL SYSCS_UTIL.SYSCS_IMPORT_TABLE(")
                .append("'APP', 'FIRM_DETAILS', '")
     ...
     private static void transferItems(final Connection newDB, 
            final Connection existingDB) throws Exception {
       ...
       final String tempFile =  Global.getTempDirectoryPath() + File.separator + 
        ...
        StringBuilder sb = new StringBuilder("{CALL SYSCS_UTIL.SYSCS_EXPORT_QUERY(")
                .append("'SELECT NAME FROM APP.ITEMS', '")
                .append(tempFile).append("', '~', null, null)}");
     ...
     private static void resetAutoIDColumn(final Connection connection, 
            final String tableName, final String columnToAlter) throws Exception {
        ...
        try (Statement s = connection.createStatement()) {
            ...
            sql = "ALTER TABLE " + tableName + " ALTER COLUMN " + 
                    columnToAlter + " RESTART WITH " + (maxID + 1);
            s.executeUpdate(sql);
        }
    }
     
      private static void transferMeasurementUnits(final Connection newDB, 
            final Connection existingDB) throws Exception {
        
       final String tempFile =  Global.getTempDirectoryPath() + File.separator + 
               "measurement_units.dat";
       Files.deleteIfExists(Paths.get(tempFile));
       
        //command to export the SELECt query contents to a disk file
        StringBuilder sb = new StringBuilder("{CALL SYSCS_UTIL.SYSCS_EXPORT_QUERY(")
                .append("'SELECT NAME, ABBREVIATION FROM APP.MEASUREMENT_UNITS', '")
                .append(tempFile).append("', '~', null, null)}");
       ...
       sb = new StringBuilder("{CALL SYSCS_UTIL.SYSCS_IMPORT_DATA(")
                .append("'APP', 'MEASUREMENT_UNITS', 'NAME,ABBREVIATION', '1,2', '")                
                .append(tempFile).append("', '~', null, null, 1)}");
    ...
    private static void transferCustomers(final Connection newDB, 
            final Connection existingDB) throws Exception {
        
        final String querySQL = "SELECT name, city, phone_numbers, " + 
                "customer_balance(cust_id, null) FROM customers";
            ...
            try(ResultSet rs = s.executeQuery(querySQL)) {
                try(PreparedStatement ps = newDB.prepareStatement(insertSQL)) {
                    while (rs.next()) {
                        ...
                        balance = rs.getBigDecimal(4); //customer's account balance
    ...
    public static boolean backupDatabase(final FinancialYear year, 
            final String path) {
        ...
        try (CallableStatement cs = connection.prepareCall(sql)) {
            ...
            logger.logp(Level.SEVERE, Database.class.getName(), "backupDatabase",
                    "Error in backing up the database", ex);
            return false;
        } finally {
            if (activeYear == null || !activeYear.equals(year)) {
    ...
    public static boolean restoreDatabase(final FinancialYear year, 
            final String restoreFromPath) {
        
        final String restoreToPath = Global.getExtendedAppDataPath() + 
                File.separator + year.toEpochMillis() + File.separator + 
                DB_NAME;
        
      final String sourcePath = restoreFromPath + File.separator + 
                year.toEpochMillis() + File.separator + DB_NAME;
        
      final String connectionString = "jdbc:derby:" + restoreToPath  
      ...
      Connection connection = null;
         ...
         try {
            ...
            logger.logp(Level.SEVERE, Database.class.getName(), "restoreDatabase",
                    "Error in restoring the database for the year " 
                            + year.toString(), e);
            return false;
        } finally {
             if (connection != null) {
     ...
     * @return - The list of names of the user defined tables in the 
     * database associated with the connection. If an error is encountered
    ...
    public static boolean compactDatabase (){
        ...
        if (tableNames == null) {return false;}
        ...
        try(CallableStatement statement = connection.prepareCall(sql)) {
            ...
            logger.logp(Level.SEVERE, Database.class.getName(), 
                    "compactDatabase", "Error in compacting the database", e);
            return false;
        }
