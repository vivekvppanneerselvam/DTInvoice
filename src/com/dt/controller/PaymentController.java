...
import com.dt.dao.PaymentPersistence;
...
public class PaymentController implements TabContent{
    ...
    public void initialize() {
        ...
        paymentModeToggle.selectedToggleProperty().addListener((ObservableValue<? extends Toggle> observable, 
            ...
            if (newValue != null && (newValue.equals(rdCheque) ||
    ...
    private boolean validateInput() {
        ...
        if (dpPaymentDate.getValue() == null) {
            ...
            if (date.isBefore(minPaymentDate)) {
                valid = false;
                lblPaymentDateError.setText("Date can't be before than " + 
                        minPaymentDate.format(dateFormatter));
            } else if (date.isAfter(maxPaymentDate)) {
                valid = false;
                lblPaymentDateError.setText("Date can't be later than " + 
        ...
        if (rdCheque.isSelected() || rdDemandDraft.isSelected()) {
    ...
    private boolean validateInstrumentDetails() {
        ...
        if (instrumentNumber.isEmpty()) {
            ...
            valid = false;
            lblInstrumentNumberError.setText("Length can't exceed 15 characters!");
        }
        ...
        if (dpInstrumentDate.getValue() == null) {
            ...
            if (date.isBefore(minInstrumentDate)) {
                valid = false;
                lblInstrumentDateError.setText("Date can't be before than " + 
                     minInstrumentDate.format(dateFormatter));
            } else if (date.isAfter(maxInstrurmentDate)) {
                valid = false;
                lblInstrumentDateError.setText("Date can't be later than " +
        ...
        if (drawnAt.length() > 100) {
            valid = false;
            lblInstrumentDrawnAtError.setText("Length can't exceed 100 characters!");
        }
    ...
    private boolean savePayment() {
        ...
        try {
            ...
            String message = "An error occurred in tyring to save the payment to the database!";
            Alert alert = new Alert(Alert.AlertType.ERROR, message,
                    ButtonType.OK);
            alert.setTitle("Error Occurred");
            alert.setHeaderText("Error in saving payment");
    ...
    */
    ...
    private void setCustomerBalance(Customer customer) {
        ...
        if (customer == null) {
            ...
            if (text == null || text.trim().isEmpty()) {
                ...
                tfCustomerBalance.setText("< invalid customer >");
            }
            return;
        }
        ...
        try {
            ...
            final String message = "An error occurred in fetching the customer's" +
                    " balance from the database.";
            final Alert alert = new Alert(Alert.AlertType.ERROR, message, ButtonType.OK);
            alert.setTitle("Error Occurred");
            alert.setHeaderText("Error in fetching customer's balance !");
            alert.initOwner(mainWindow);
            Utility.beep();
             Global.styleAlertDialog(alert);
            alert.showAndWait();
        ...
        if (error) {
            tfCustomerBalance.setText("Error!!");
        } else {
            if (this.paymentId != 0) {
    ...
    private ButtonType shouldSaveUnsavedData() {
        String message = "The payment is unsaved.\n"
                + "Would you like to save it before closing this window?";
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION, message,
                ButtonType.YES, ButtonType.NO, ButtonType.CANCEL);
        alert.setTitle("Unsaved Payment");
        alert.setHeaderText("Save payment before closing window?");
        alert.initOwner(mainWindow);
         Global.styleAlertDialog(alert);
    ...
    public boolean loadPayment(int paymentId) {
        ...
        try {
            ...
            String message = "An error occurred in fetching the payment" +
                    " details from the database!";
            Alert alert = new Alert(Alert.AlertType.ERROR, message, ButtonType.OK);
            alert.setHeaderText("Error in fetching payment details!");
            alert.setTitle("Error Occurred");
            alert.initOwner(mainWindow);
            Utility.beep();
             Global.styleAlertDialog(alert);
