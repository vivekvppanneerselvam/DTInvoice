...
import com.dt.dao.AccountLookup;
import com.dt.dao.CustomersPersistence;
import com.dt.dao.InvoiceSearch;
import com.dt.dao.PaymentSearch;
import com.dt.dto.AccountEntry;
import com.dt.dto.AccountLookupCriteria;
import com.dt.dto.Customer;
import com.dt.dto.EntityEditedEvent;
import com.dt.dto.EntityEditedListener;
import com.dt.dto.Invoice;
import com.dt.dto.InvoiceItem;
...
public class CustomerAccountController implements TabContent, EntityEditedListener {
    ...
    private static final Logger logger
            = Logger.getLogger(CustomerAccountController.class.getName());
...
//    private ComboBox<Customer> cbCustomers;
    ...
    public void initialize() {
        errorLabels = new Label[]{
            lblCustomersError, lblEntriesSinceError
        };
        ...
        tableView.getSelectionModel().selectedItemProperty().addListener(
                (ObservableValue<? extends AccountEntry> observable,
                        AccountEntry oldValue, AccountEntry newValue) -> {
...
//        cbCustomers.requestFocus();
    ...
    private void onEntriesLookupAction(ActionEvent event) {
        ...
        try {
            ...
            final String message = "An error occurred in tyring to fetch customer's "
                    + "account's details from the database!";
            Alert alert = new Alert(Alert.AlertType.ERROR, message,
                    ButtonType.OK);
            alert.setTitle("Error Occurred");
            alert.setHeaderText("Error in fetching data");
        ...
        final ObservableList<AccountEntry> observableList
                = FXCollections.<AccountEntry>observableList(entries,
                        (AccountEntry param) -> {
                            Observable[] array = new Observable[]{
                                param.entryDateProperty(), param.entryDescriptionProperty(),
    ...
    private boolean validateInput() {
        ...
        if (chkEntriesSince.isSelected()) {
            ...
            if (entrySince == null) {
                ...
                lblEntriesSinceError.setText("Selected date can't precede "
                        + minDate.format(dateTimeFormatter));
                valid = false;
            } else if (entrySince.isAfter(maxDate)) {
                lblEntriesSinceError.setText("Selected date can't be later than "
    ...
    private void setTableCellFactories() {
        dateColumn.setCellFactory(new Callback<TableColumn<AccountEntry, LocalDate>, TableCell<AccountEntry, LocalDate>>() {
...
//        List<Customer> customers = null;
        ...
        try {
            ...
            Alert alert = Utility.getErrorAlert("Error Occurred",
                    "Error in Fetching Customers Data !", message, mainWindow);
            Utility.beep();
            alert.showAndWait();
            return false;
        }
...
//        cbCustomers.setItems(customerList);
    ...
    private StringBinding getAccountBalanceBinding(final ObservableList<AccountEntry> list) {
        final StringBinding binding = new StringBinding() {
            ...
            protected String computeValue() {
                ...
                return (IndianCurrencyFormatting.applyFormatting(balance.abs())
                        + suffix);
            }
        };
    ...
    public void onEntityEdited(EntityEditedEvent event) {
        ...
        if (entityEdited instanceof Invoice) {
            ...
            entry.setEntryDescription(payment.getPaymentMode().toString()
                    + " Payment");
    ...
    private BigDecimal getInvoiceTotal(Invoice invoice) {
        ...
        for (InvoiceItem item : invoiceItems) {
            amount = item.getQuantity().multiply(item.getRate()).setScale(2,
    ...
    private void onEditEntryAction(ActionEvent event) {
        ...
        try {
            ...
            Alert alert = Utility.getErrorAlert("Error Occurred",
                    "Error Occurred in opening the " + text,
                    "Could not open the " + text + " due to an error.", mainWindow);
            Utility.beep();
            alert.showAndWait();
            return;
        }
    ...
    private void onDeleteEntryAction(ActionEvent event) {
        ...
        String header = null;
        if (invoiceId != 0) {
            message = "Are you sure you want to delete this invoice numbered "
                    + invoiceId + " and dated "
        ...
        try {
            ...
            String content = "An error occurred in deleting the "
                    + (invoiceId != 0 ? "invoice" : "payment") + ".";
            Alert dialog = Utility.getErrorAlert("Error Occurred", "Error in Deletion",
                    content, mainWindow);
            Utility.beep();
            dialog.showAndWait();
            return;
    ...
    private void onPrintAccountAction(ActionEvent event) {
        ...
        final String resourcePath = "/resources/reports/fxbilling_customer_account.jasper";
        ...
        try (InputStream reportStream = this.getClass().getResourceAsStream(resourcePath)){
            ...
            logger.logp(Level.SEVERE, CustomerAccountController.class.getName(),
                    "onPrintAccountAction", "Error in fillReport function call", e);
            Utility.beep();
            final String message = "An error occurred whilst preparing to print the Customer's account!";
            final Alert alert = new Alert(Alert.AlertType.ERROR, message,
                    ButtonType.OK);
            alert.setTitle("Error Occurred");
            alert.setHeaderText("Error in printing account statement");
            alert.initOwner(mainWindow);
             Global.styleAlertDialog(alert);
            alert.showAndWait();
            return;
...
//         try {
//            JasperPrintManager.printReport(jasperPrint, false); //false means don't show the print dialog. Send output directly to the default printer
//        } catch (Exception ex) {
