...
import com.dt.dao.PaymentSearch;
import com.dt.dto.Customer;
import com.dt.dto.EntityEditedEvent;
import com.dt.dto.EntityEditedListener;
import com.dt.dto.Payment;
import com.dt.dto.PaymentMode;
import com.dt.dto.PaymentSearchCriteria;
import com.dt.dto.PaymentSearchResult;
import com.dt.utils.IndianCurrencyFormatting;
...
public class PaymentsSearchController implements TabContent, EntityEditedListener {
    ...
    private final static Logger logger = 
            Logger.getLogger(PaymentsSearchController.class.getName());
    ...
    private final LocalDate minPaymentDate = Global.getActiveFinancialYear()
            .getStartDate();
    private final LocalDate maxPaymentDate = Utility.minDate(Global.getActiveFinancialYear()
   ...
   private CheckBox chkCustomers;
    ...
    @FXML
    private CheckListView lvwCustomers;
     @FXML
    ...
    @FXML Button btnEditPayment;
    @FXML Button btnDeletePayment;
    ...
    public void initialize() {
        ...
        chkRealisedPaymentsOnly.managedProperty()
                .bind(chkRealisedPaymentsOnly.visibleProperty());
        chkRealisedPaymentsOnly.visibleProperty().bind(chkCheque.selectedProperty()
                .or(chkDD.selectedProperty()));
        ...
        errorLabels = new Label[]{
            lblPaymentDateError, lblCustomersError, lblPaymentModeError
        };

        for (Label label : errorLabels) {
            label.managedProperty().bind(label.visibleProperty());
            label.visibleProperty().bind(label
                    .textProperty().length().greaterThan(0));
        }
        ...
        btnEditPayment.disableProperty().bind(tableView.getSelectionModel()
                .selectedItemProperty().isNull());
        btnDeletePayment.disableProperty().bind(tableView.getSelectionModel()
                .selectedItemProperty().isNull());
    ...
    @FXML
        ...
        try {
            ...
            Alert alert = new Alert(Alert.AlertType.ERROR, "An error occurred " +
                    "whilst searching for the matching payments", ButtonType.OK);
            alert.setHeaderText("Error Occurred");
            alert.setTitle("Error in searching for payments");
        ...
        final ObservableList<PaymentSearchResult> listItems = 
                FXCollections.<PaymentSearchResult>observableList(results, 
                        (PaymentSearchResult param) -> {
                Observable[] array = new Observable[] {
                    param.amountProperty(), param.customerProperty(),
      ...
      final  StringBinding summaryBinding = new StringBinding() {
            ...
            protected String computeValue() {
                ...
                String output = String.format("The total of %d payments is \u20B9 %s",
                        listItems.size(), 
        ...
        if (resultSize > 0) {
            ...
            Alert alert = new Alert(Alert.AlertType.INFORMATION, 
                    "No payment matched your search criteria!", ButtonType.OK);
            alert.setTitle("No matching payment");
       ...
       clearErrorLabels();
        ...
        if (chkBetweenDates.isSelected()) {
            if(dpStartDate.getValue() == null || dpEndDate.getValue() == null) {
                ...
                lblPaymentDateError.setText("Start date can not precede " + 
                        minPaymentDate.format(dateFormatter));
                valid = false;
            } else if (dpEndDate.getValue().isAfter(maxPaymentDate)) {
                lblPaymentDateError.setText("End date can not be later than " + 
                        maxPaymentDate.format(dateFormatter));
                valid = false;
            } else if (dpStartDate.getValue().isAfter(dpEndDate.getValue())) {
                lblPaymentDateError.setText("Start date can't be later than the end date!");
                valid = false;
            }
        }
        ...
        if (chkPaymentMode.isSelected()) {
            if (! (chkCash.isSelected() || chkCheque.isSelected() || 
                    chkDD.isSelected() || chkBankTransfer.isSelected())) {
    ...
    private DateCell getDateCell(DatePicker datePicker) {
      ...
      private PaymentSearchCriteria getPaymentCriteria() {
        ...
        if (chkPaymentMode.isSelected()) {
            ...
            if (chkCheque.isSelected() || chkDD.isSelected()) {
    ...
    private void setTableCellFactories() {
        ...
        paymentDateColumn.setCellFactory((TableColumn<PaymentSearchResult, 
                LocalDate> param) -> {
            TableCell<PaymentSearchResult, LocalDate> tableCell
                    = new TableCell<PaymentSearchResult, LocalDate>() {
                        @Override
        ...
        amountColumn.setCellFactory((TableColumn<PaymentSearchResult, BigDecimal> param) -> {
            TableCell<PaymentSearchResult, BigDecimal> tableCell = 
                    new TableCell<PaymentSearchResult, BigDecimal>() {
        ...
        paymentModeColumn.setCellFactory(new Callback<TableColumn<PaymentSearchResult, PaymentMode>, TableCell<PaymentSearchResult, PaymentMode>>() {

            @Override
            public TableCell<PaymentSearchResult, PaymentMode> call(TableColumn<PaymentSearchResult, PaymentMode> param) {
    ...
    private void onDeletePaymentAction(ActionEvent event) {
      ...
      int paymentId = item.getPaymentId();
        try {
            ...
            String message = "An error occurred in deleting the payment from the database.";
            Alert alert = new Alert(Alert.AlertType.ERROR, message, ButtonType.OK);
            alert.setTitle("Error Occurred");
            alert.setHeaderText("Error in deleting the payment");
            alert.initOwner(mainWindow);
            Utility.beep();
            alert.showAndWait();
            return;
    ...
    private boolean shouldDeletePayment(PaymentSearchResult item) {
       ...
       String message = "Are you sure to delete this payment dated " + 
               date.format(formatter) + " and amounting to \u20B9" + 
               IndianCurrencyFormatting.applyFormatting(amount) + " ?";
       Alert alert = new Alert(Alert.AlertType.CONFIRMATION, message, 
               ButtonType.YES, ButtonType.NO);
       alert.setTitle("Sure to delete?");
       alert.setHeaderText("Sure to delete this payment?");
       alert.initOwner(mainWindow);
    ...
    private void onEditPaymentAction(ActionEvent event) {
      ...
      PaymentSearchResult searchResult =  tableView.getSelectionModel()
              .getSelectedItem();
      int paymentId  = searchResult.getPaymentId();
      
       String path =  "/com/dt/view/Payment.fxml";
       FXMLLoader loader = new FXMLLoader();
        ...
        try {
            ...
            logger.logp(Level.SEVERE, this.getClass().getName(), 
                    "onEditPaymentAction", "Error in loading the payment fxml file", e);
           Alert alert = Utility.getErrorAlert("Error Occurred", 
                   "Error Occurred in opening the payment",
                   "Could not open the payment due to an error.", mainWindow);
           Utility.beep();
           alert.showAndWait();
           return;
        }
    ...
    public void onEntityEdited(EntityEditedEvent event) {
        ...
        if (mode == PaymentMode.CHEQUE || mode == PaymentMode.DD) {
